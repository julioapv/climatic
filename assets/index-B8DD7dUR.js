(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))o(e);new MutationObserver(e=>{for(const r of e)if(r.type==="childList")for(const s of r.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function i(e){const r={};return e.integrity&&(r.integrity=e.integrity),e.referrerPolicy&&(r.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?r.credentials="include":e.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function o(e){if(e.ep)return;e.ep=!0;const r=i(e);fetch(e.href,r)}})();const g=({city:t,currentTemp:n,feelsLike:i,condition:o,icon:e,humidity:r,windSpeed:s,sunrise:l,sunset:d})=>{const a=document.createElement("section"),m=`/assets/icons/${e}.png`,h=t.charAt(0).toUpperCase()+t.slice(1).toLowerCase();return a.innerHTML=`
    <section id="current-weather" class="bg-big-stone rounded-lg p-6 mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-xl">Feels like: ${i}°C</p>
                    <p class="text-lg">${o}</p>
                    <img src="${m}" alt="icon ${e} icon" class="w-16 h-16">
                    </div>
                <div class="md:text-xl md:flex md:flex-col md:gap-2 ">
                    <p>Humidity: ${r}%</p>
                    <p>Wind Speed: ${s} km/h</p>
                    <p>Sunrise: ${l} AM</p>
                    <p>Sunset: ${d} PM</p>
                </div>
            </div>
        </section>
    `,a},f=({date:t,maxTemp:n,minTemp:i,weatherCondition:o,icon:e,precipProb:r,windSpeed:s})=>{const l=document.createElement("div"),d=`/assets/icons/${e}.png`,a=t.replace("2025-","");return l.innerHTML=`
    ...
    `,l},y=t=>{const n=document.createElement("section");n.innerHTML=`
            ...
            `;const i=n.querySelector(".grid");return t.slice(0,5).forEach(o=>{i.appendChild(f(o))}),n},b=()=>{const t=document.createElement("div"),n="bg-rose-900 rounded-lg p-10".split(" ");return t.classList.add(...n),t.innerHTML=`
    ...
    `,t},c=document.querySelector("#app"),p=document.querySelector("#location-input"),v=document.querySelector("#search-query-button");v.addEventListener("click",()=>{p.value&&(L(),u(x()))});const x=()=>{let t=p.value;return p.value="",t};function L(){c.innerHTML=""}async function u(t){const n=C();c.appendChild(n);try{const i=await fetch(`https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${t}?unitGroup=metric&key=DSFV7FNKTT4KEK5QWBJQXQTJR&contentType=json`);if(!i.ok)throw new Error("Failed to fetch data");const o=await i.json();n.remove();let e={city:o.address,currentTemp:o.currentConditions.temp,feelsLike:o.currentConditions.feelslike,condition:o.currentConditions.conditions,icon:o.currentConditions.icon,humidity:o.currentConditions.humidity,windSpeed:o.currentConditions.windspeed,sunrise:o.currentConditions.sunrise,sunset:o.currentConditions.sunset};const r=o.days.map(s=>({date:s.datetime,maxTemp:s.tempmax,minTemp:s.tempmin,weatherCondition:s.conditions,icon:s.icon,precipProb:s.precipprob,windSpeed:s.windspeed}));c.appendChild(g(e)),c.appendChild(y(r))}catch(i){console.error(i),c.appendChild(b()),n.remove()}}u("Cologne");
